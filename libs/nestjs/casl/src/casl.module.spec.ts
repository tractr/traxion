...
describe('Populate on fly when getSelectPrismaUserQuery is used', () => {
  let app: INestApplication;
  let mockUser: MockProxy<{
    getUser: () => Record<string, unknown> | null;
    isPublic: () => boolean;
  ...
  it('should hydrate the user with the select params before creates the abilities', async () => {
    mockUser.getUser.mockReturnValue({
      id: 'test',
      roles: ['admin'],
    });
    mockAuthenticationUserService.findUnique.mockReturnValue(
      Promise.resolve({
        id: 'test',
        roles: ['admin'],
    ...
    expect(mockAuthenticationUserService.findUnique).toHaveBeenCalledWith({
      where: {
        id: 'test',
      },
      select: selectPrismaUserQuery,
    });
  });
});
