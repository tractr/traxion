...
export function discoverOwnership(
  ...
  const ownedModels: OwnedModel[] = schema.relations
    .filter(
      (relation) =>
    ...
    .map((relation) => {
      switch (relation.type) {
        case 'oneOne':
        case 'oneMany':
          if (relation.from.model.name === model.name) {
            return {
    ...
    .filter(
      ...
      (ownedModel) => ownedModel.own.metadata?.ownership !== 'ignore',
    )
    .filter(
      // We filter owned models that are already explored
      (ownedModel) => !explored.find((m) => m.name === ownedModel.own.name),
    )
    .map((ownedModel) => ({
      ...ownedModel,
      own: { ...ownedModel.own, ownedModels: [] },
    }))
