...
export function generateServiceClass(
  ...
  const className = `${pascal(model.name)}${pascal(field.name)}Service`;

  const methods = [generateValidateMethod(model, field)];
...
export function generateServiceSourceFile(
  ...
  const imports = [
    ...
    generateImport('AngularModelsValidatorsModule', [
