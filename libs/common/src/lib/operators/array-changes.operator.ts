 ...
 *
 * Usage: stream$.pipe(arrayChanges())
 */
export function arrayChanges<T>(): OperatorFunction<T[], ArrayChanges<T>> {
  return function operator(source) {
    return source.pipe(
      scan(
        (acc, curr) => {
          // Get removed items
          const removed = acc.value.filter((item) => !curr.includes(item));

          // Get added items
          const added = curr.filter((item) => !acc.value.includes(item));
